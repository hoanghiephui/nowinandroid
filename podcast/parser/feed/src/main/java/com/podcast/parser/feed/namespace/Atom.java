...
import com.podcast.model.feed.FeedFunding;
import com.podcast.model.feed.FeedItem;
import com.podcast.model.feed.FeedMedia;
import com.podcast.parser.feed.HandlerState;
import com.podcast.parser.feed.element.AtomText;
import com.podcast.parser.feed.element.SyndElement;
import com.podcast.parser.feed.util.DateUtils;
import com.podcast.parser.feed.util.MimeTypeUtils;
import com.podcast.parser.feed.util.SyndStringUtils;
...
public class Atom extends Namespace {
        ...
        if (ENTRY.equals(localName)) {
            ...
            if (parent.getName().matches(isFeedItem)) {
                if (rel == null || LINK_REL_ALTERNATE.equals(rel)) {
                    ...
                    String mimeType = MimeTypeUtils.getMimeType(attributes.getValue(LINK_TYPE),
                            href);

                    FeedItem currItem = state.getCurrentItem();
                    if (MimeTypeUtils.isMediaFile(mimeType) && currItem != null
                            && !currItem.hasMedia()) {
                ...
                if (rel == null || LINK_REL_ALTERNATE.equals(rel)) {
                    ...
                    if (state.getFeed() != null &&
                            ((type == null && state.getFeed().getLink() == null) ||
                                    (LINK_TYPE_HTML.equals(type) || LINK_TYPE_XHTML.equals(
                                            type)))) {
    ...
    public void handleElementEnd(String localName, HandlerState state) {
        ...
        if (state.getTagstack().size() >= 2) {
            ...
            } else if (PUBLISHED.equals(top) && ENTRY.equals(second)
                    && state.getCurrentItem() != null) {
                state.getCurrentItem().setPubDate(DateUtils.parseOrNullIfFuture(content));
            } else if (IMAGE_LOGO.equals(top) && state.getFeed() != null
                    && state.getFeed().getImageUrl() == null) {
